...
+ (UIImage *)resizeToMax:(UIImage *)image max:(CGFloat)max
{
    ...
    CGFloat imageLong = width > height ? width : height;
    ...
    {
        targetWidth = max;
        targetHeight = max * height / width;
    }
    else
    {
        targetHeight = max;
        targetWidth = max * width / height;
    }
    ...
    NSLog(@"resizeToMax: %f %f To: %f %f Result: %f %f", width, height, targetWidth, targetHeight, newImage.size.width, newImage.size.height);
    return newImage;
}
