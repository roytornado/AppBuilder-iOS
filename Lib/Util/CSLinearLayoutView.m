...
- (void)layoutSubviews
{
	...
	if (_autoAdjustContentSize == YES) {
		...
		if (self.orientation == CSLinearLayoutViewOrientationHorizontal) {
			...
			contentWidth	= self.contentDelta;
			self.bounds		= CGRectMake(0, 0, contentWidth, self.bounds.size.height);
		} else {
			CGFloat contentHeight = MAX(self.frame.size.height, self.contentDelta);
			contentHeight	= self.contentDelta;
			self.bounds		= CGRectMake(0, 0, self.bounds.size.width, contentHeight);
		}
