...
- (void)addSubviewsFromStartV:(CGFloat)pad views:(NSArray *)views
{
    ...
    if ([self isKindOfClass:[UIScrollView class]])
    {
        ...
        [scrollView setContentSize:CGSizeMake(self.width, curY)];
    }
    else
    {
        self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, self.width, curY);
    }
}
...
- (void)addSubviewsFromStartH:(CGFloat)pad views:(NSArray *)views
{
    ...
    {
        UIScrollView *scrollView = (UIScrollView *)self;
        [scrollView setContentSize:CGSizeMake(curX, self.height)];
    }
    else
    {
        self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, curX, self.height);
    }
}
